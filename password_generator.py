...
def frame2_children(frame, frame3):
    ...
    find_password_button = Button(frame, text="find site password",
                                  command=lambda: find_password_for_site(find_password_entry))
...
def authenticated_window(user=None):
    top1 = Toplevel()
    top1_w = 550  # width for the Tk root
    top1_h = 450  # height for the Tk root
    top1_height = int(top1.winfo_screenheight())
    top1_width = int(top1.winfo_screenwidth())
    set_top1_width = (top1_width / 2) - (top1_w / 2)
    set_top1_height = (top1_height / 2) - (top1_h / 2)
    top1.geometry('%dx%d+%d+%d' % (top1_w, top1_h, set_top1_width, set_top1_height))
    ...
    Label(frame, text=f"Welcome {user.capitalize()}", anchor="center", font=('Helvetica', 14), pady=10).grid(row=0,
                                                                                                             column=0,
                                                                                                             columnspan=3)
...
def get_insert_signup_data(user, pwd, window):
    if user.get() and (any(char.isdigit() for char in pwd.get()) and len(pwd.get()) >= 8):
        user_name = user.get()
        user_pwd = pwd.get()
        user.delete(0, "end")
        pwd.delete(0, "end")
        ...
        messagebox.showwarning("Invalid input",
                               "Fill all fields and password must be 8 characters long and contain a number")
...
top_height = int(top.winfo_screenheight())
top_width = int(top.winfo_screenwidth())
set_width = (top_width / 2) - (w / 2)
set_height = (top_height / 2) - (h / 2)
top.geometry('%dx%d+%d+%d' % (w, h, set_width, set_height))
print(top_height, top_width)
